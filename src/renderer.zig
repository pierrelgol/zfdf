...
pub const RendererConfig = struct {
    ...
    pub fn init(map_data: *const MapData, camera: *const Camera) RendererConfig {
...
pub const Renderer = struct {
    ...
    pub inline fn render(renderer: *Renderer) void {
        ...
        while (y < height) : (y += 1) {
            ...
            while (x < width) : (x += 1) {
                const world_point = Vec3{
                    .x = @floatFromInt(x),
                    .y = @floatFromInt(y),
                    .z = @floatFromInt(world[(y) * width + (x)]),
                };
                var zoomed_point = world_point.mulScalar(config.cam_zoom);
                zoomed_point.z *= config.cam_scale;
    ...
    pub inline fn draw(renderer: *Renderer) void {
        const height = renderer.config.height;
        const width = renderer.config.width;
        assert(height != 0);
        assert(width != 0);
    ...
    pub inline fn drawPixel(renderer: *const Renderer, pixel: Pixel) void {
        renderer.mlx_backend.putPixelImage(pixel.x, pixel.y, pixel.color);
    }

    pub inline fn drawLine(renderer: *Renderer, start: Pixel, end: Pixel) void {
        ...
        while (true) {
            renderer.drawPixel(.{ .x = x, .y = y, .color = start.color });
